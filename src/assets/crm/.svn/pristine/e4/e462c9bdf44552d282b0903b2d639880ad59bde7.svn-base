            ...
            <c:forEach items="${order.lines }" var="line">
            	<c:if test="${line.id!=0 and line.id!=null }">
            		 <tr>
		                ...
		                <td>${line.num }</td>
		                <td>${line.num }</td>
		                <td><input name="attribute3"  value="${line.num }"/></td>
		                <td><input  name="attribute4" value="${line.attribute4 }"/></td>
		                <td><input name="id" value="${line.id }" type="hidden" /></td>
		                <input type="hidden" name="deliverynum" value="${line.deliverynum }">
		            </tr>
            	</c:if>
            </c:forEach>
        </table>
...
$(function(){
	...
	$("#saveBtn").click(function(){
		renameLogistics();
		$("#saveForm").attr("action","customerInvAllocation/submitMakeSure.html").submit();
	})
});
