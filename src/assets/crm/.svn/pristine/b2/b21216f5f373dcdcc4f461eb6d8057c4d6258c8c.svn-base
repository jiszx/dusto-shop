...
$(function() {
	...
	/*var agentTypeUrl = "customer/contract/lines?id=" + $("#contractId").val();
	$.get(agentTypeUrl, function(data) {
		if (data.rows && data.rows.length > 0) {
			$("#agent-type").val(data.rows[0].agentType);
			$("#agentType").val(data.rows[0].agentType);
		}
		realoadAgent();
	});*/
});
        ...
        $.getJSON("product/categroy/categroys.json",{},function (data) {
            if(data.length =0){
                ...
                for(var i=0;i<item.length;i++){
                	...
                	/*else if(type=='2' && item[i].id.length==baseLength+4){
                		$("#agentId").append("<option value='"+item[i].id+"'>"+ 
                				findParent(item,item[i].pid)+item[i].name+"</option>")
                	}else if(type=='3' && item[i].id.length==baseLength+2){
                		$("#agentId").append("<option value='"+item[i].id+"'>"+ 
                				findParent(item,item[i].pid)+item[i].name+"</option>")
                	}*/
                }
