...
$(function() {
	...
	$("#editnum").blur(function(){
		...
		var hbNum =parseFloat($("#edithbNum").val());
		if(!hbNum){
			hbNum=0;
		}
	...
	//合资盐业公司下仓储服务商
//	var url = 'order/util/customer?custType=70&all=1';
//	$.get(url, function(data) {
//		if (data.rows && data.rows.length > 0) {
//			customerdata=data.rows;
//			for (var i = 0; i < data.rows.length; i++) {
...
//	});
	...
	$("#rdcCode").change(function(){
		...
		//type 等于1表示要验证碘标
		var materialurl = "order/util/custProductByRdc?merchid=" + merchid+"&orgid="+$("#orgid").val()+"&combination=1"+"&rdcCode="+rdcCode+"&type="+$("#type").val();
...
function doEditSave(){
	...
	if(validateDecimal($("#edithbNum").val()) ==false){
		$.messager.popup("货补数量只能输入大于0数字，输入小于三位小数");
		return ;
	}
	var orderPrice =$("#editorderPrice").val();
	var amt = parseFloat(orderPrice*num);//调拨金额
	var hbAmt = parseFloat(orderPrice*hbNum);//货补金额
...
function doSave() {
	...
	var rdcCode =$("#rdcCode").val();
	var receiveRDC =$("#type").val()=='2'?'Z901':'';
	var deliveryType =$("#deliveryType").val();
