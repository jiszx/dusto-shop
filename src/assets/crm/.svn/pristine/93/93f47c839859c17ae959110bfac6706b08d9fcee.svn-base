...
function addAgent(agent) {
	...
	linediscountamt = (parseFloat(linediscountamt)
			+ parseFloat($("#discountAmt").val())).toFixed(2);
...
function changeTotalAndamt() {
	...
	//$("#btn-add-agent").css('display','none'); 
}
function validateNum(){
	...
	var amounts =parseFloat($("#amounts").val());
	var ordernum=parseFloat($("#num").val());
	if(!ordernum){
		ordernum=0;
	}
	var hbnum=parseFloat($("#hbNum").val());
	if(!hbnum){
		hbnum=0;
	}
	var num =(ordernum+hbnum)*amounts;
	if(!num){
		num=0;
	}
...
function numchangeamt(){
	...
	//$("#btn-add-agent").css('display','block'); 
}
