...
@Service("reduceRetailOrderDepo")
@Transactional
public class ReduceRetailOrderDepoTask implements JavaDelegate {
  ...
  public ReduceResult reduceByOrder(Long id) {
    ...
    /*// 获取合同数据
    CMerchCustContract contract = reduceDepo.getContract(header);*/
    CMerchCustBase merch = this.custbaseMapper.selectByPrimaryKey(header.getShipId());
    // 获取订单行数据
    ...
    for (OmOrderSpilts line : lines) {
      // 更新客户库存数据
      int reduceResult = stockService.reduceProductInvByOrder(header, line,merch, period);
      if (reduceResult == -1) {
        logger.error("订单扣减客户库存，库存不足 orderid:{} merch:{} material:{} org:{}", header.getId(), 
        		merch.getId(), line.getMaterialId(), header.getOrganizationId());
