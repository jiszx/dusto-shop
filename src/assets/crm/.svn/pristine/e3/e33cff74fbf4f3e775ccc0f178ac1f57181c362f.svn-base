...
$(function() {
	...
	var url = 'order/util/customer?type=1&custType=4';
...
function changeTotalAndamt() {
	...
	if (parseFloat(lineorderamt) < parseFloat(cashAmt)) {
		...
		$("#XJamt").html(cashAmt);
		$("#SXamt").html(lineorderamt - cashAmt);
	}
...
function doSave() {
	...
	for(var i=0;i<data.length;i++){
		if(data[i].sku !='合计'){
			...
			data[i].price =parseFloat(data[i].price/data[i].amounts).toFixed(4);
			data[i].orderPrice =parseFloat(data[i].orderPrice/data[i].amounts).toFixed(4);
		}else{
			data.splice(i,1);
		}
	}
