...
<div class="form-group col-md-4 col-sm-6">
		...
		<div class="col-md-7 col-sm-11">
			<select class="form-control no-appearance" name="merchLevels" disabled="disabled">
				<c:if test="${adjust.accountType=='5' }">
					<option>授信金额</option>
				</c:if>
				<c:if test="${adjust.accountType !='5' }">
					<c:forEach items="${dict['MERCH_ACCOUNT_TYPE']}" var="it">
						<c:if test="${it.chooseVal !='4'}">
					 		<option value="${it.chooseVal}" ${it.chooseVal == adjust.accountType?"selected":""}>${adjust.accountType=='3'?"授信额度":it.showText}</option>
						</c:if>
					</c:forEach>
				</c:if>
				
		   </select>
			<%-- <input type="text" class="form-control"  readonly="readonly" value="${adjust.accountType }"> --%>
		</div>
...
<div class="form-group col-md-4" class="col-sm-6 ">
		...
		<div class="col-md-7 col-sm-11">
			<input type="text" class="form-control"  readonly="readonly" value="${adjust.amt}">
		</div>
