...
function doSearch() {
	...
	var url = "order/cancel/cancelList?custname=" + customer + "&bdate="
			+ bdate + "&edate=" + edate+"&crmorderid="+crmorderid+"&saporderid="+saporderid;
...
function doDetail() {
	...
	if (rows && rows.length == 1) {
		window.location.href = "order/orderdetails.html?id="
				+ rows[0].orderHeaderId;
	} else {
...
function doCancel() {
	var rows = $("#orderTable").bootstrapTable("getSelections");
	if (rows && rows.length == 1) {
		var url = "order/close/doClose?id=" + rows[0].orderHeaderId;
...
function doSendSap() {
	$.post("order/cancel/doClose?id=" + $("#orderid").val(), function(res) {
		$.messager.alert("提示", res.data.msg);
		$("#orderTable").bootstrapTable("refresh");
	});
}
