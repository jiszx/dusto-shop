$(function() {
	$('#adjustTable').bootstrapTable({
		...
		url: 'account/adjust/list?accountType=4&custType=70',
		cache : false,
		//data : data,
		toolbar : "#dictTool",
		striped : true,
	...
	$("#organizationId").change(function(){
		...
		var url = 'account/util/customerByType?type=70&orgid='+orgid;
...
function doEdit() {
	...
	if (rows && rows.length == 1 && (rows[0].states =="1" || rows[0].states =="4")) {
			$('#editmerchCustId').empty();
			$("#editmerchCustId").trigger("chosen:updated");
			var url = 'account/util/customerByType?type=70&orgid='+rows[0].organizationId;
			$.get(url, function(data) {
				if (data.rows && data.rows.length > 0) {
					$('#editmerchCustId').append("<option></option>");
