$(function () {
    ...
    $("#saveBtn").click(function(){
    	var len = $('div[id^=distribution_]').length;
    	var type = $('input[name="custType"]:first').val();
    	// 仓储服务商
    	if(isTransferMerch(type) && len<2){
    		$.messager.popup("请添加送货地址信息");
    		return;
    	}
    	if(validate("#saveForm").form()){
	    	distributionsRename();
	    	$("#saveForm").attr("action","customer/save.html").submit();
    	}
    })
...
function distributionFormToCard() {
    ...
    $("#addRecevingModal [id^='form_distribute_input']").val('');
}
...
function isTransferMerch(type){
	if(!type){
		return false;
	}
	if(type=='7' || type=='70' || type=='8'){
		return true;
	}
	return false;
}
