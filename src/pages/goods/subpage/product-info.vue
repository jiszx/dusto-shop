...
<template>
  <div class="subpage-product-info">
    ...
    <div>
      <span class="red inline-block marginR5">￥{{data.price}}</span>
      <span class="gray inline-block line-through old_price" v-if="data.old_price">￥{{data.old_price}}</span>
    </div>
    <div v-if="data.Integral">
      <p class="red">{{data.Integral}}积分可兑换</p>
    </div>
    <div>
      <ul class="clearfix color-type-list">
        <li
